...
const G = { baseSpeed: 1, maxSpeed: 12.2, speed: 1, laneCount: 4, speedUpRate: 0.002, accel: 0.0008, spawnDelayMs: 700, minSpawnDelayMs: 380, spawnGapY: 160, maxObstacles: 6 };
...
function update(){
  frame++;
  // Time-based spawning with gap and count limits
  const now = performance.now();
  const canSpawnCount = obstacles.length < G.maxObstacles;
  let nearestY = Infinity;
  for (let i = 0; i < obstacles.length; i++) nearestY = Math.min(nearestY, obstacles[i].y);
  const hasGap = nearestY > G.spawnGapY || obstacles.length === 0;
  // Gradually reduce spawn delay as speed increases (but clamp to minimum)
  const dynamicDelay = Math.max(G.minSpawnDelayMs, G.spawnDelayMs - (G.speed * 20));
  if (canSpawnCount && hasGap && (now - lastSpawnAtMs) >= dynamicDelay) {
    spawnObstacle();
    lastSpawnAtMs = now;
  }
